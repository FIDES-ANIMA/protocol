...
const ADAPTER_DIR_BY_PROFILE: Record<string, string> = {
  cursor: "harness/cursor/adapter",
  "claude-code": "harness/claude-code/adapter",
  codex: "harness/codex/adapter",
};
  ...
  if (args.json) {
    ...
    if (!dispatcherLayerActive) {
      ...
      if (args.profile === "openclaw" || args.profile === "generic") {
        ...
        console.log(
          `  See harness/${args.profile}/adapter/ and harness/${args.profile}/runbook.md`,
        );
      }
    }
    if (!trustLayerActive) {
