...
/**
 * Stage the OpenClaw ClawHub skill and return the flattened skill root.
 * Assurance inventories must not scan the git monorepo.
 */
export async function resolveSkillAssuranceDir(repoRoot: string): Promise<string> {
  const { stageSkill } = await import(
    "../harness/openclaw/scripts/stage-skill.js"
  );
  const result = stageSkill({
    repoRoot,
    outDir: join(repoRoot, "harness", "openclaw", "skill-dist"),
  });
  return result.outDir;
}

/**
 * Inventory the flattened staged skill (ClawHub publish root), excluding
 * stage metadata and installed deps.
 */
export function inventoryStagedSkill(skillDir: string): PackageInventory {
  const entries = walkFiles(skillDir)
    .map((f) => f.path)
    .filter(
      (p) => p !== ".skill-stage.json" && !p.startsWith("node_modules/"),
    );
  return inventoryAndChecksums(skillDir, entries);
}
...
async function main(): Promise<void> {
  ...
  const skillDir = await resolveSkillAssuranceDir(root);
  const skillInv = inventoryStagedSkill(skillDir);
  const skillInvPath = join(outDir, "skill.inventory.json");
  writeFileSync(skillInvPath, JSON.stringify(skillInv, null, 2) + "\n", "utf8");
  const skillSbomPath = join(outDir, "skill.cdx.json");
  await generateSbom(skillDir, skillSbomPath);
  console.log(`Wrote ${skillInvPath} (${skillInv.files.length} files) and ${skillSbomPath}`);
  ...
  const targets = [
    ...
    { dir: join(root, "harness", "openclaw", "plugin"), label: "plugin" },
    { dir: join(root, "harness", "openclaw", "plugin-trust"), label: "plugin-trust" },
  ];
