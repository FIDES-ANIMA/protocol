...
stage_skill() {
  ...
  (cd "$REPO_ROOT" && npx tsx harness/openclaw/scripts/stage-skill.ts --out "$out" --install-deps)
}
...
if [[ "$UPDATE_TEST_MODE" -ne 1 ]]; then
  ...
  if [[ "$WANT_PLUGIN" -eq 1 ]]; then
    pack_asset "harness/openclaw/plugin" "$STAGE_ROOT/plugin"
  fi
  if [[ "$WANT_TRUST" -eq 1 ]]; then
    pack_asset "harness/openclaw/plugin-trust" "$STAGE_ROOT/plugin-trust"
  fi
  if [[ "$WANT_CURSOR" -eq 1 ]]; then
    pack_asset "harness/cursor/adapter" "$STAGE_ROOT/cursor"
  fi
  if [[ "$WANT_CLAUDE" -eq 1 ]]; then
    pack_asset "harness/claude-code/adapter" "$STAGE_ROOT/claude-code"
  fi
  if [[ "$WANT_CODEX" -eq 1 ]]; then
    pack_asset "harness/codex/adapter" "$STAGE_ROOT/codex"
  fi
fi
...
if [[ "$WANT_PLUGIN" -eq 1 ]]; then
  summarize_target "plugin" "$PLUGIN_DIR" "$REPO_ROOT/harness/openclaw/plugin/package.json"
fi
if [[ "$WANT_TRUST" -eq 1 ]]; then
  summarize_target "trust" "$TRUST_DIR" "$REPO_ROOT/harness/openclaw/plugin-trust/package.json"
fi
if [[ "$WANT_CURSOR" -eq 1 ]]; then
  summarize_target "cursor-adapter" "$CURSOR_DIR" "$REPO_ROOT/harness/cursor/adapter/package.json"
fi
if [[ "$WANT_CLAUDE" -eq 1 ]]; then
  summarize_target "claude-adapter" "$CLAUDE_DIR" "$REPO_ROOT/harness/claude-code/adapter/package.json"
fi
if [[ "$WANT_CODEX" -eq 1 ]]; then
  summarize_target "codex-adapter" "$CODEX_DIR" "$REPO_ROOT/harness/codex/adapter/package.json"
fi
printf '\n'
