...
} from "../harness/openclaw/plugin/src/risk-classifier.ts";
...
function main() {
  ...
  if (json) {
    ...
    for (const profile of adapterProfiles) {
      const pkg = resolve(root, "harness", profile, "adapter", "package.json");
      console.log(
        `  ${existsSync(pkg) ? "[PRESENT]" : "[MISSING]"} ${profile} → harness/${profile}/adapter`,
      );
    }
    console.log(
...
if (!existsSync(resolve(root, "harness", "openclaw", "plugin", "src", "risk-classifier.ts"))) {
  console.log(
