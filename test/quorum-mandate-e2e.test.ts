...
import { MandateStore } from "../harness/openclaw/plugin/src/mandate-store.ts";

import { loadOrCreateIdentity } from "../harness/openclaw/plugin-trust/src/identity.ts";
import { KeyLifecycleLedger } from "../harness/openclaw/plugin-trust/src/key-lifecycle.ts";
import { parseQuorumPolicyConfig } from "../harness/openclaw/plugin-trust/src/quorum-policy.ts";
import {
  QuorumSessionManager,
  computeIntendedMandateDigest,
  signQuorumBallot,
  signQuorumProposal,
} from "../harness/openclaw/plugin-trust/src/quorum-session.ts";
...
describe("quorum mandate e2e", () => {
  ...
  it("finalize → store → unattended allow + debit + quorum-mandate receipt", async () => {
    ...
    } = await import("../harness/openclaw/plugin/src/index.ts");
    const { createHookCapture } = await import("../harness/openclaw/plugin/src/test-helpers.ts");
