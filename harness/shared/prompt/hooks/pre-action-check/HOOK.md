...
This file (`HOOK.md`) is a navigational aid, not a runnable script. The runnable plugin lives in this git monorepo at `harness/openclaw/plugin/src/index.ts` (not shipped in the ClawHub skill).

## If you really wanted a runnable script…

…you probably meant one of:

1. The plugin entry: `harness/openclaw/plugin/src/index.ts`. Build with `cd harness/openclaw/plugin && npm install && npm run build`.
2. The verify-install script: `scripts/verify-install.ts`. Run with `npm run verify-install -- --soul <path> --memory <path>` from the skill root.
3. The self-test script: `scripts/self-test.ts`. Run with `npm run self-test` from the skill root.
