...
STAGE_DIR="harness/openclaw/skill-dist"
STAGE_SKILL="harness/openclaw/scripts/stage-skill.ts"
SKILL_SELF_CHECK="harness/openclaw/scripts/skill-self-check.ts"
...
EXAMPLES:
  ...
  bash harness/openclaw/scripts/clawhub-publish.sh publish skill --changelog "Fix adoption docs"

  # Bump plugin to specific version without publishing
  bash harness/openclaw/scripts/clawhub-publish.sh bump plugin --version 1.2.0

  # Publish everything with minor bump
  bash harness/openclaw/scripts/clawhub-publish.sh publish all --bump minor --changelog "Add trust metrics"

  # Check current versions
  bash harness/openclaw/scripts/clawhub-publish.sh status
USAGE
  exit 0
}
...
get_skill_version() {
  sed -n 's/^version: *//p' "$REPO_ROOT/$SKILL_MD" | tr -d '[:space:]'
}
...
set_skill_version() {
  local new_ver="$1"
  local skill="$REPO_ROOT/$SKILL_MD"
...
run_strict_checks_skill() {
  ...
  (cd "$REPO_ROOT" && npx tsx "$STAGE_SKILL" --out "$STAGE_DIR" --install-deps)
  (cd "$REPO_ROOT" && npx tsx "$SKILL_SELF_CHECK" --root "$STAGE_DIR")
  green "  ✓ Skill checks passed (verify + stage + install-deps + skill-self-check)"
}

stage_skill_dist() {
  bold "Staging OpenClaw-only skill → $STAGE_DIR/..."
  (cd "$REPO_ROOT" && npx tsx "$STAGE_SKILL" --out "$STAGE_DIR" --install-deps)
  [[ -f "$REPO_ROOT/$STAGE_DIR/SKILL.md" ]] || die "$STAGE_DIR/SKILL.md missing after stage"
  [[ -f "$REPO_ROOT/$STAGE_DIR/package.json" ]] || die "$STAGE_DIR/package.json missing after stage"
  green "  ✓ $STAGE_DIR staged (runtime deps installed for verify)"
}
...
do_bump_skill() {
  ...
  if [[ -f "$REPO_ROOT/$SKILL_PKG" ]]; then
    set_json_version "$REPO_ROOT/$SKILL_PKG" "$new_ver"
    green "  ✓ $SKILL_PKG → $new_ver"
  fi
...
publish_skill() {
  ...
  if [[ "$DRY_RUN" != "true" ]]; then
    set_json_version "$REPO_ROOT/$STAGE_DIR/package.json" "$ver"
    if grep -q '^version:' "$REPO_ROOT/$STAGE_DIR/SKILL.md"; then
      sed -i.bak "s/^version: .*/version: $ver/" "$REPO_ROOT/$STAGE_DIR/SKILL.md"
      rm -f "$REPO_ROOT/$STAGE_DIR/SKILL.md.bak"
    fi
  fi

  bold "Publishing skill v${ver} from $STAGE_DIR/..."
  if [[ "$DRY_RUN" == "true" ]]; then
    yellow "  [dry-run] clawhub skill publish $STAGE_DIR --slug $SKILL_SLUG --name \"$SKILL_DISPLAY_NAME\" --version $ver --changelog \"$changelog\" --owner $OWNER"
    return
  fi

  (cd "$REPO_ROOT" && clawhub skill publish "$STAGE_DIR" \
    --slug "$SKILL_SLUG" \
