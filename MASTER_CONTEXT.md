...
see `README.md` and `harness/shared/prompt/SKILL.md`.
...
| Status (Sep 2025 → May 2026) | 1.0.0 — historical: `production_ready`, 39/39 tests passing at that time | 1.3.2 skill, 1.2.2 trust plugin, 1.1.4 enforcement plugin (source: `package.json`, `harness/openclaw/plugin-trust/package.json`, `harness/openclaw/plugin/package.json`) |
...
This is summarized in `README.md` and `harness/shared/prompt/SKILL.md`; reproduced here for offline
context.
...
`active` / `inactive` / `unknown`. Cross-harness adapters ship under `harness/<harness>/adapter/`
with graded guarantees (`harness/shared/harness-capabilities.json`); gateway-non-bypassable
...
The current repo's `harness/openclaw/plugin-trust/` module is the live implementation of three of
...
|---|---|---|---|
| **Constitutional Identity (CID)** — Ed25519 / ECDSA agent identity derived from constitutional commitment | `src/core/constitutional-identity.ts` | `harness/openclaw/plugin-trust/src/identity.ts` | Implemented in both |
...
| **Trust Graph Protocol (TGP)** — directed scoped trust; policy-constrained propagation; self/peer views; signed event ledger | `src/core/trust-graph-protocol.ts` | `harness/openclaw/plugin-trust/src/trust-graph.ts` + `trust-scope.ts` + `trust-views.ts` + `trust-events.ts` + `persistence.ts` | Implemented in trust plugin (Plan 7 contextual trust) |
| **Constitutional Handshake Sequence (CHS)** — multi-step verification; signed claim exchange; Merkle root exchange | `src/core/handshake-sequence.ts` | `harness/openclaw/plugin-trust/src/handshake.ts` + `claims.ts` + `merkle-bridge.ts` | Implemented in trust plugin |

This is the *conceptual* architecture. When reading `harness/openclaw/plugin-trust/` code, mapping
...
- **`harness/openclaw/plugin-trust/` requires Node `>=22.19`.** Older Node versions need
  `--ignore-engines`.
...
3. **Cross-runtime parity.** Prompt layer works across AgentSkills harnesses.
   ...
   (graded — see `harness/shared/harness-capabilities.json`). Gateway-non-bypassable
...
- `harness/shared/prompt/SKILL.md` — the prompt-layer skill (agent-facing; the
  body addresses the agent in second person).
...
- `harness/openclaw/skill-dist/` — generated OpenClaw-only ClawHub skill root
  (gitignored; produced by `harness/openclaw/scripts/stage-skill.ts`).
