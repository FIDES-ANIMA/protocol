...
Machine-readable matrix: `harness/shared/harness-capabilities.json`. Runbooks: `harness/<harness>/runbook.md`.

### Prompt-layer skill

The **ClawHub** skill is an **OpenClaw** prompt-layer package (allowlisted stage via `harness/openclaw/scripts/stage-skill.ts`). It does not include Cursor / Claude Code / Codex adapters. Those live in this git monorepo under `harness/<harness>/adapter/` and are installed from a clone.
...
| Claude Code | yes | Place under `.claude/skills/` or `~/.claude/skills/`. Adapter: `harness/claude-code/adapter/`. |
...
On Claude Code, Cursor, and Codex, install the matching adapter under `harness/<harness>/adapter/` and wire the sample hooks config. Be explicit about graded guarantees:
...
If you do not set these, sensible defaults apply (see `harness/openclaw/plugin/src/config.ts` and `harness/openclaw/plugin-trust/openclaw.plugin.json`). Manifest path defaults may remain relative (`.openclaw/workspace/...`) for readability; **runtime** `mergeConfig` / `mergeTrustConfig` absolutize them under `<homedir>/.openclaw/workspace` (or `FPP_WORKSPACE` when set — use this when OpenClaw `workspaceDir` is non-default). The trust plugin uses `fallbackAuditLogPath` when `constitution-audit.jsonl` has no entries yet, so handshakes can bootstrap from enforcement audit activity before the first heartbeat (or before `npm run audit:bootstrap`). Set `fallbackAuditLogPath` to `null` if you run trust without the enforcement plugin. Empty trust graphs do not write `fpp-trust-graph.json` until the first mutation (see TROUBLESHOOTING §0a3).

Default `knownCustomTools` is empty (operator extras only). OpenClaw live names normalize (`openclawfpp_*` → `fpp_*`, `openclaw.<name>` → `<name>`, curated/gateway remainders) before classify. Named allows: `internal.heartbeat`, `internal.read`, `gateway.inspect`, `fpp.governance`. `apply_patch` → `code.patch` → approval (never default-allowlisted). `exec.benign` allows without staged-action ledger rows. **Neither proves behavioral compliance** — only configuration/event evidence.
...
4. **Heartbeat skills are model-driven.** The constitution-audit heartbeat is a sub-skill the agent runs voluntarily. Operators can also create/extend the primary log without the model via `npm run audit:bootstrap -- --soul <path>` (same adoption/revocation gates). Suggested cron: every 6 hours invoking that script. Until OpenClaw exposes a native unconditional scheduler, completeness still benefits from either bootstrap cron or continued heartbeat cooperation.
