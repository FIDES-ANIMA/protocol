...
1. Configure the trust plugin's `stewardAuthorizationLedgerPath` and `stewardInstanceAudience` keys exactly as declared in `harness/openclaw/plugin-trust/openclaw.plugin.json`, then restart the gateway process so it reloads the manifest schema:
