...
| Cursor | [`harness/cursor/runbook.md`](../harness/cursor/runbook.md) | `--profile cursor` |
| Claude Code | [`harness/claude-code/runbook.md`](../harness/claude-code/runbook.md) | `--profile claude-code` |
| Codex | [`harness/codex/runbook.md`](../harness/codex/runbook.md) | `--profile codex` |

Capability matrix: [`harness/shared/harness-capabilities.json`](../harness/shared/harness-capabilities.json). Compatibility: [`docs/COMPATIBILITY.md`](COMPATIBILITY.md).
...
**Maintainers:** `npx tsx harness/openclaw/scripts/stage-skill.ts --install-deps` (used by `clawhub-publish.sh`) installs runtime deps into `harness/openclaw/skill-dist/` before self-check. `npx tsx harness/openclaw/scripts/skill-self-check.ts --root .` validates that staged tree — it must not pass merely because the monorepo root declares workspaces.
...
**Code path:** `harness/openclaw/plugin-trust/src/index.ts` calls `api.registerCli(..., { descriptors: FPP_TRUST_CLI_DESCRIPTORS })` where descriptors name is `fpp-trust` (`harness/openclaw/plugin-trust/src/cli.ts`). Registration runs inside the OpenClaw **plugin host** during gateway/plugin load — not as a standalone CLI package.
...
- Line endings only: an LF checkout hashes as `bc764d3c…` even though the JSON text is unchanged. The signed seed is the CRLF serialization. Confirm `.gitattributes` contains `constitution.json text eol=crlf` and that the working-tree file contains `\r\n`.
...
- The plugin's manifest declares hooks the runtime version doesn't support. Compare `openclaw --version` with the `pluginApi` field in `harness/openclaw/plugin/package.json`'s `openclaw.compat`. If your runtime is older, you must either upgrade OpenClaw or pin the plugin to an older version.
