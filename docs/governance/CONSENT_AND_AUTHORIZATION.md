...
**Implementation pointer (honest labels):** Plan 8 folds `StandingMandateV1` and disposition/authorization literals into `@fides-anima/fpp-protocol-core` and wires an unattended disposition engine in the enforcement plugin. That maps **operator authorization**, **delegated/standing mandates**, and **emergency allow-minimal + mandatory review** into tool-boundary decisions. Plan 9 adds peer/steward **quorum issuance** of signed mandates (`plugin-trust` propose/second/finalize), but **rejects** finalize when the proposal scope claims `affected-party-consent` / `data-subject-consent` (or aliases) — agent majority still cannot manufacture nonparticipant consent. The 2026-07-18 steward-operator path adds OpenPGP-signed `OperatorAuthorizationV1` artifacts (distinct from Ed25519 `StandingMandateV1` wire signatures) that normalize to the same `issuerClass: "operator"` disposition coverage — still **not** third-party consent. Labels below remain `PROVISIONAL` for full consent-collection engines and guardian instruments.
